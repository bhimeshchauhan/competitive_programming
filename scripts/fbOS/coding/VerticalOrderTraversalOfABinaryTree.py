...
class Solution():
    def verticalTraversal(self, root):
        ...
        queue = [(root, 1, 0)]
        # use a dictionary to record column idx: (depth, node value)
        ...
        # finally sort the column idx in dictionary to get the order in result
        columns = list(dic.keys())
        columns.sort()
        # sort each column by its depth (x[0]) and value (x[1])
        # => sort nodes by their values in each row
        dic = [sorted(dic[c], key=lambda x: (x[0], x[1])) for c in columns]
        result = []
        for elements in dic:
            col = []  # each col
            for _, val in elements:
                col.append(val)  # get only the value
            result.append(col)
        return result
