...
as a string array and given an array of string pairs similarPairs 
where similarPairs[i] = [xi, yi] indicates that the two words xi 
and yi are similar.
...
the similarity relation is transitive. For example, if the words 
a and b are similar, and the words b and c are similar, then 
a and c are similar.
...
Output: true
Explanation: The two sentences have the same length and each word
i of sentence1 is also similar to the corresponding word in sentence2.
...
"""
Time Complexity: O(NP) where N is the maximum length 
of words1 and words2, and P is the length of pairs.
