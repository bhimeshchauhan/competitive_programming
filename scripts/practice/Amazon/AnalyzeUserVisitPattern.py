...
of users x such that x visited "leetcode" then visited "love" and visited "leetcode" one more time after that.
Also, if the pattern is ["luffy", "luffy", "luffy"], the score is the number 
of users x such that x visited "luffy" three different times at different timestamps.
...
class Solution:
    def mostVisitedPattern(self, username: List[str], timestamp: List[int], website: List[str]) -> List[str]:
        ...
        for user, time, site in sorted(zip(username, timestamp, website), key=lambda x: (x[0], x[1])):
            # defaultdicts simplify and optimize code
            users[user].append(site)

        # this can also be replaced with a manually created dictionary of counts
        patterns = Counter()
