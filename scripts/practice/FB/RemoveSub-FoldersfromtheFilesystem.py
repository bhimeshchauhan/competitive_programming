...
Output: ["/a","/c/d","/c/f"]
Explanation: Folders "/a/b/" is a subfolder of "/a" and "/c/d/e" 
is inside of folder "/c/d" in our filesystem.
...
class Solution():
    def removeSubfolders(self, folder: List[str]) -> List[str]:
        ...
        i = 1
        while(i<len(folder)):
            if ans[-1] not in folder[i]: # folder strings are substrings of subfolders
                ans.append(folder[i])
            else:
                length = len(ans[-1]) # if condition meets, check whether folder[i][length] is equal to "/"
                                    # "/a/b/c","/a/b/ca" here is the example 
                if folder[i][length] != "/":
                    ans.append(folder[i])
            i+=1
