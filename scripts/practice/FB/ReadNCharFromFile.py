...
class Solution:
    def __init__(self):
        # Holds excess characters read from file between queries
        self.queue = collections.deque([])
    
    def read(self, buf: List[str], n: int) -> int:
        # Fill-up queue until we can answer the query
        # (or until end-of-file reached)
        while len(self.queue) < n:
            local4Buf = [None] * 4
            # Read up to 4 chars
            ret = read4(local4Buf)
            for val in local4Buf:
                if val is not None:
        ...
        tmp = []
        # Return n chars (or less if end-of-file was reached)
        while len(tmp) < n and len(self.queue) > 0:
