...
def length_of_longest_substring(arr, k):
  ...
  for window_end in range(len(arr)):
    ...
    # Current window size is from window_start to window_end, overall 
    # we have a maximum of 1s
    # repeating 'max_ones_count' times, this means we can have a 
    # window with 'max_ones_count' 1s
    # and the remaining are 0s which should replace with 1s.
    # now, if the remaining 0s are more than 'k', it is the time 
    # to shrink the window as we
